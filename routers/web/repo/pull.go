...
func MergePullRequest(ctx *context.Context) {
	...
	if !confirmedMerge && !form.ForceMerge {
		...
		if err == nil {
			...
			if err == nil {
				...
				if hasNoChecks || hasFailedChecks {
					if hasNoChecks {
						ctx.JSONError(ctx.Tr("repo.pulls.merge_no_ci_confirm_desc"))
					} else {
						ctx.JSONError(ctx.Tr("repo.pulls.merge_failed_checks_confirm_desc"))
					}
					return
				}
			}
